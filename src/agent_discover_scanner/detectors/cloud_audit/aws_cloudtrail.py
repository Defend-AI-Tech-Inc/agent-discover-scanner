"""
AWS CloudTrail and CloudTrail Lake detectors — Layer 5 Cloud Audit (AWS).
...
Both feed into Layer 5 via run_cloudtrail_detection(). Findings carry:
...
def _normalise_event(
    *,
    ...
    """Return a Layer 5-compatible finding dict for a single CloudTrail event."""
...
def fetch_cloudtrail_events(
    ...
    Returns a list of normalised Layer 5 findings.
...
def fetch_cloudtrail_lake_events(
    ...
    Returns a list of normalised Layer 5 findings with source="cloudtrail_lake".
    """
    import boto3
    import botocore.exceptions  # type: ignore[import]
...
def run_cloudtrail_detection(
    ...
    if lake_arn:
        try:
            ...
            _cred_msg = "CloudTrail scan skipped: boto3 not installed"
            logger.warning(_cred_msg)
            if _warn_fn:
                _warn_fn(_cred_msg)
            ...
            if _resp:
                ...
                if code in ("AccessDeniedException", "AuthFailure", "UnauthorizedOperation"):
                    logger.warning(
                        ...
                        "grant cloudtrail:LookupEvents to enable Bedrock Layer 5 detection",
                        code,
                    )
                    return results
            if "NoCredentialsError" in type(exc).__name__ or "credentials" in str(exc).lower():
                _cred_msg = "CloudTrail scan skipped: no AWS credentials"
                logger.warning(_cred_msg)
                if _warn_fn:
                    _warn_fn(_cred_msg)
                return results
...
# ---------------------------------------------------------------------------
# CloudAuditDetector implementation
# ---------------------------------------------------------------------------


class AWSCloudTrailDetector(CloudAuditDetector):
    """
    Layer 5 Cloud Audit detector for AWS via CloudTrail.

    Wraps run_cloudtrail_detection() / fetch_cloudtrail_lake_events() in the
    CloudAuditDetector ABC so auto-discovery in detectors/cloud_audit/__init__.py
    can enumerate and run it alongside future Azure and GCP detectors.
    """
    ...
    def __init__(
        ...
        self._region = region
        self._lake_arn = lake_arn
        self._lake_timeout_seconds = lake_timeout_seconds

    def is_available(self) -> bool:
        """Return True when boto3 is installed (credentials checked lazily)."""
        try:
    ...
    def detect(self, hours_back: int) -> list[CloudAuditFinding]:
        """
        Run CloudTrail detection and return typed CloudAuditFinding instances.

        Internally delegates to run_cloudtrail_detection() which returns raw
        dicts; this method wraps each dict in a CloudAuditFinding.
        """
        raw_findings = run_cloudtrail_detection(
            lookback_hours=hours_back,
            lake_arn=self._lake_arn,
            region=self._region,
            lake_timeout_seconds=self._lake_timeout_seconds,
        )
        results: list[CloudAuditFinding] = []
        for f in raw_findings:
            try:
                event_time_raw = f.get("timestamp") or ""
                try:
                    event_time = datetime.fromisoformat(event_time_raw)
                except (ValueError, TypeError):
                    event_time = datetime.now(timezone.utc)

                results.append(CloudAuditFinding(
                    provider="aws",
                    service=f.get("provider") or "bedrock",
                    event_name=f.get("event_name") or "",
                    event_time=event_time,
                    principal=f.get("username") or "",
                    source_ip=f.get("source_ip") or "",
                    model_id=f.get("model_id"),
                    framework_hint=f.get("framework"),
                    source=f.get("source") or "cloudtrail_historical",
                ))
            except Exception:
                continue
        return results
